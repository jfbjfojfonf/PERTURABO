...
---
> **Deux flux PUR, ne pas les confondre :**
>
> | | PUR long-form (CE guide) | PUR direct (guides 13 / 14 / 19 / 20) |
> |---|---|---|
> | Entrée | podcast + transcript dans `PUR/IN/` | F00B (Twitch) **ou** F00C (YouTube) → `candidats.json` |
> | Orchestrateur | `pur.py` (4 gates) | `pur_adapter_direct` → F04 → `pur_montage_pipeline` |
> | F01 / F02 / F03 | utilisés | **SKIPPÉS** (VOX a déjà sélectionné) |
> | Assembleur | GATE 4 via **F05_PACKAGER** | F06 inline (`pur_montage_pipeline.py`), F05 absent |
> | Quand | long-form / podcast, tu veux les 4 gates | clip viral depuis un live/VOD, le plus court chemin |
>
> Ce fichier décrit **uniquement** le flux long-form. Pour VOX → pack, ouvre le guide 13.
---
## 🎯 Le but en 3 lignes
