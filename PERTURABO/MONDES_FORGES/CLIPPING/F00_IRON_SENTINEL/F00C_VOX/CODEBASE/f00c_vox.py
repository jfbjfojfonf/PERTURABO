...
def _yt_dlp_json(url: str, cookies_path: Path | None = None,
    ...
    if shutil.which("yt-dlp") is None:
        return None
    cookie_args = ["--cookies", str(cookies_path)] if cookies_path and Path(cookies_path).is_file() else []
    # --ignore-no-formats-error : depuis fin 2025, YouTube exige un PO token
    # (runtime JS) pour lister les formats — sans lui yt-dlp avorte AVANT de
    # lire la heatmap. La barre rouge est dans le JSON même sans formats :
    # on exige donc seulement la lecture, pas la liste des formats.
    attempts = [
        ["yt-dlp", "--no-playlist", "--dump-json", "--ignore-no-formats-error",
         "--extractor-args", f"youtube:player_client={player_clients}"] + cookie_args,
        ["yt-dlp", "--no-playlist", "--dump-json", "--ignore-no-formats-error"] + cookie_args,
    ]
    for attempt in attempts:
        attempt.append(url)
        try:
